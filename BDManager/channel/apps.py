...
def DBtoHive(request):
  ...
  for case in switch(conMode):
    if case('normal'):
      shell = "sqoop import --connect jdbc:mysql://"+DBConf['URL']+":"+DBConf['port']+"/"+DBConf['DBName']+" --username "+DBConf['account']+" --password "+DBConf['passwd']+" --table "+DBTable+" --hive-import --hive-table "+hiveTable+" --columns "+sqoopConf['columns']+" --where"+sqoopConf['where']+" --split-by "+sqoopConf['spliteKey']
      print(shell)
      os.system(shell)
      res['success'] = True
